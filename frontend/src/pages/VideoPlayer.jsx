...
function VideoPlayer() {
  ...
  const load = async () => { setLoading(true);setShowAd(true); try { const [v,r]=await Promise.all([videoAPI.getById(id),videoAPI.getAll({limit:6})]); setVideo(v.data);setRelated(r.data.videos?.filter(x=>(x._id||x.id)!==id)||[]); } catch(e){toast.error('Failed');} finally{setLoading(false);} };
  ...
  return (<div className="video-page"><div className="container"><div className="video-layout">
    <div className="video-player-section">
      ...
      :<div className="player-wrapper"><video src={videoUrl} controls autoPlay poster={thumbnailUrl}/></div>}
    ...
    <aside className="video-sidebar"><AdBanner type="sidebar"/><h3 className="sidebar-title">Related</h3>
      {related.map(v=><Link to={`/video/${v._id||v.id}`} key={v._id||v.id} className="related-video"><div className="related-thumb"><img src={v.thumbnailUrl||v.thumbnail_url||'https://images.unsplash.com/photo-1611162616475-46b635cb6868?w=200'} alt={v.title}/></div><div className="related-info"><h4>{v.title}</h4><span>{v.views?.toLocaleString()} views</span></div></Link>)}
